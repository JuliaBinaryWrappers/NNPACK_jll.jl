...
if VERSION < v"1.3.0-rc4"
    ...
    error("Unable to import NNPACK_jll on Julia versions older than 1.3!")
end
...
artifacts = Pkg.Artifacts.load_artifacts_toml(artifacts_toml; pkg_uuid=UUID("a6bfbf70-4841-5cb9-aa18-3a8ad3c413ee"))
platforms = [Pkg.Artifacts.unpack_platform(e, "NNPACK", artifacts_toml) for e in artifacts["NNPACK"]]
...
if best_platform === nothing
    @debug("Unable to load NNPACK; unsupported platform $(triplet(platform_key_abi()))")
else
    # Load the appropriate wrapper
    include(joinpath(@__DIR__, "wrappers", "$(best_platform).jl"))
end

end  # module NNPACK_jll
